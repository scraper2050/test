...
export const generateIncomeReport = async (params: any, division?: DivisionParams) => {
  try {
    const response: any = await request('/generateIncomeReport', 'OPTIONS', params, undefined, undefined, undefined, division);
...
export const getMemorizedReports = async (division?: DivisionParams) => {
  try {
    const response: any = await request('/getMemorizedReports', 'OPTIONS', undefined, undefined, undefined, undefined, division);
    const { status, message } = response.data;
    if (status === 1) {
      return response.data;
    }
...
export const generateIncomePdfReport = async (params: any, division?: DivisionParams) => {
  ...
  try {
    const response: any = await request(`/generateIncomeReportPdf${queryParams}`, 'GET', {}, undefined, undefined, undefined, division);
    return response.data;
  } catch {
    return { 'status': 0,
      'message': `Something went wrong` };
...
export const generateAccountReceivableReport = async (type: number, asOf: string, customerIds?: string[], division?: DivisionParams) => {
  ...
  try {
    const response: any = await request(`/generateAccountReceivableReport${params}`, 'GET', {}, undefined, undefined, undefined, division);
    return response.data;
  } catch {
    return { 'status': 0,
      'message': `Something went wrong` };
...
export const generateAccountReceivablePdfReport = async (type: number, asOf: string, customerIds?: string[], division?: DivisionParams) => {
  ...
  const params = `?reportData=${type}&asOf=${asOf}${ids ? `&customerIds=${ids}` : ''}`;
  try {
    const response: any = await request(`/generateReportPdf/AccountReceivableReport${params}`, 'GET', {}, undefined, undefined, undefined, division);
    return response.data;
  } catch {
    return { 'status': 0,
      'message': `Something went wrong` };
