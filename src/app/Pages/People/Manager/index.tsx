...
const ManagerPage = (): JSX.Element => {
  ...
  useEffect(() => {
    Api.post('/getManagers', null)
      .then((res) => {
        if (res.data.status === 1) setManagerList([...res.data.users]);
      })
      .catch((err) => {
        console.log('Get managers err:', err);
      });
  }, []);
