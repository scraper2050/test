...
function InvoicingListListing({ classes, theme }: any) {
  ...
  useEffect(() => {
    ...
    dispatch(getAllInvoicesAPI(undefined, undefined, undefined, undefined, advanceFilterInvoiceData, undefined, undefined, undefined, undefined,undefined,undefined, currentDivision.params));
    return () => {
      dispatch(setKeyword(''));
  ...
  useEffect(() => {
    if (fetchInvoices) {
      dispatch(getAllInvoicesAPI(currentPageSize, undefined, undefined, keyword, advanceFilterInvoiceData, undefined, undefined, undefined,undefined,undefined,undefined, currentDivision.params));
      dispatch(setCurrentPageIndex(0));
    }
  ...
  useEffect(() => {
    if (location?.state?.tab === 1 && (location?.state?.option?.search || location?.state?.option?.pageSize || location?.state?.option?.lastPrevCursor
      ...
      dispatch(getAllInvoicesAPI(location.state.option.pageSize, location?.state?.option?.lastPrevCursor, location?.state?.option?.lastNextCursor, location.state.option.search, advanceFilterInvoiceData, undefined, undefined, undefined,undefined,undefined,undefined, currentDivision.params));
  ...
  const handleFilterSubmit = async (data: any) => {
    ...
    const { total } = await (getAllInvoicesAPI(currentPageSize, undefined, undefined, keyword, data, undefined, undefined,undefined,undefined,undefined, undefined, currentDivision.params))(dispatch);
