...
function DashboardPage({ classes }: any): JSX.Element {
  ...
  const RenderStatus = ({ status }: StatusTypes) => {
    ...
    return (
      <div
        className={
          status ? classes.statusConfirmedText : classes.statusPendingText
        }
      >
        <div
          className={
            status ? classes.statusConfirmedCircle : classes.statusPendingCircle
          }
        />
        <div className={classes.statusText}>{textStatus}</div>
      </div>
    );
  };
